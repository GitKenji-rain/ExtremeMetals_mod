...
import com.surubedai.extrememetals.blockentity.BlockentitySteamLathe;
...
public class ScreenSteamLathe extends AbstractContainerMenu {
    public final BlockentitySteamLathe blockEntity;
    private final Level level;
    private final ContainerData data; // 進捗バーの同期用データ

    // クライアント側（画面を開く側）が呼び出すコンストラクタ
    public ScreenSteamLathe(int containerId, Inventory inv, FriendlyByteBuf extraData) {
    ...
    public ScreenSteamLathe(int containerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ExtremeMetalsAddScreens.STEAM_LATHE_MENU.get(), containerId);
        checkContainerDataCount(data, 3);
        this.blockEntity = ((BlockentitySteamLathe) entity);
