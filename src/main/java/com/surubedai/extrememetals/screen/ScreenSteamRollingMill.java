...
public class ScreenSteamRollingMill extends AbstractContainerMenu {
    public final BlockentitySteamRollingMill blockEntity;
    private final Level level;
    private final ContainerData data; // 進捗バーの同期用データ

    // クライアント側（画面を開く側）が呼び出すコンストラクタ
    public ScreenSteamRollingMill(int containerId, Inventory inv, FriendlyByteBuf extraData) {
    ...
    public ScreenSteamRollingMill(int containerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ExtremeMetalsAddScreens.STEAM_ROLLING_MENU.get(), containerId);
        checkContainerDataCount(data, 3);
        this.blockEntity = ((BlockentitySteamRollingMill) entity);
